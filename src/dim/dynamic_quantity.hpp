...
{
    return dynamic_unit{(int8_t) -u[0], (int8_t) -u[1], (int8_t) -u[2], (int8_t) -u[3], (int8_t) -u[4], (int8_t) -u[5], (int8_t) -u[6], (int8_t) -u[7]};
}
...
struct dynamic_quantity : public dynamic_quantity_tag {
    ...
    constexpr dynamic_quantity(Q const& i_q)
        : value(i_q.value)
        , unit{toDynamic<typename Q::unit>()}
    { }

    template<class Q, DIM_IS_QUANTITY(Q)>
    constexpr Q as() const
    {
        if(unitsMatch(unit, toDynamic<typename Q::unit>())) {
            return Q(value);
        }
    ...
    template<class S2>
    friend dynamic_quantity<S, System> multiply(dynamic_quantity<S, System> const& a, dynamic_quantity<S2, System> const& b)
    {
        dynamic_quantity<S, System> c;
        c.value= a.value*b.value;
        for(unsigned int i=0; i<a.unit.size(); i++) {
            c.unit[i] = a.unit[i] + b.unit[i];
        }
        return c;
    }

    template<class S2>
    friend dynamic_quantity<S, System> divide(dynamic_quantity<S, System> const& a, dynamic_quantity<S2, System> const& b)
    {
        dynamic_quantity<S, System> c;
        c.value = a.value/b.value;
        for(unsigned int i=0; i<a.unit.size(); i++) {
    ...
    friend dynamic_quantity<S, System> add(dynamic_quantity<S, System> const& a, dynamic_quantity<S2, System> const& b)
    {
        if(unitsMatch(a.unit, b.unit)) {
            return dynamic_quantity<S, System>(a.value + b.value, a.unit);
        }
        return dynamic_quantity<S, System>::bad_quantity();
    }

    template<class S2>
    friend dynamic_quantity<S, System> subtract(dynamic_quantity<S, System> const& a, dynamic_quantity<S2, System> const& b)
    {
        if(unitsMatch(a.unit, b.unit)) {
            return dynamic_quantity<S, System>(a.value - b.value, a.unit);
        }
    ...
    template<class S2>
    friend dynamic_quantity<S, System> operator+(dynamic_quantity<S, System> const& a, dynamic_quantity<S2, System> const& b)
    {
        return add(a, b);
    }
    template<class S2>
    ...
    template<class S2>
    friend dynamic_quantity<S, System>& operator+=(dynamic_quantity<S, System>& a, dynamic_quantity<S2, System> const& b)
    {
        if(unitsMatch(a.unit, b.unit)) {
            a.value+= b.value;
            return a;
